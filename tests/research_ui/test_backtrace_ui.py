...
state.originTrace = {steps: [step('8', 'observed'), step('7', 'correlated')],
  gaps: [{after_step: 1, reason: 'missing_event', detail: 'Earlier event was dropped.'}],
  coverage: {linked_steps: 1, observed_links: 1, correlated_links: 0, gap_count: 1, percent: 50}};
...
const coverage = elements.coverageValue.textContent;
rows[2].click();
...
state.requests = [live]; state.selectedRequestId = null; state.originTrace = null; renderBacktrace();
const chooseCaptured = elements.traceEmptyTitle.textContent;
process.stdout.write(JSON.stringify({empty, loading, selection, inert, identifiers, coverage, gap, retained, unrelatedHidden, missingId, chooseCaptured}));
"""
        ...
        self.assertTrue(actual["identifiers"])
        self.assertEqual(
            actual["coverage"],
            "50% predecessor coverage · 1 of 2 predecessor links recorded · 1 missing predecessor",
        )
        self.assertEqual(actual["missingId"], "This event has no request identifier")
        self.assertEqual(actual["chooseCaptured"], "Choose a captured request")
